...
describe('Overview', () => {
 ...
 it('Verify METRICS Navigation for Lambda', () => {
   ...
   jazzServices_po.getService(servicename).click();
   commonUtils.fluentwaittry(jazzServices_po.getServiceNameHeader(), fivek);
   commonUtils.elementPresent(jazzServices_po.getProdName(), tenk);
   jazzServices_po.getProdName().click();
   commonUtils.waitForSpinnerDisappear();
   commonUtils.refreshbutton(jazzServices_po.getProdHeader(), fivek);
   browser.driver.switchTo().activeElement();
   ...
   commonUtils.waitForSpinnerDisappear();
   commonUtils.fluentwaittry(jazzServices_po.getTestFunction(), fivek);
 ...
 it('Verify METRICS COUNT for Lambda', () => {
  ...
  //  commonUtils.waitForSpinnerDisappear();
   ...
   jazzServices_po.getMetrices().click();
   commonUtils.waitForMetricsSpinner();
   commonUtils.refreshbutton(jazzServices_po.getMetricesCount(), fivek);
 ...
 it('Verify METRICS Navigation for Lambda for Test Branch', () => {
   browser.sleep(twok);
   commonUtils.fluentwaittry(jazzServices_po.getTestFunction(), fifteenk);
   expect(jazzServices_po.getTestFunction().getText()).toEqual('TEST FUNCTION');
   jazzServices_po.getTestFunction().click();
   browser.wait(EC.visibilityOf(jazzServices_po.getTestArea()), timeOutHigh);
   jazzServices_po.getTestArea().sendKeys('{');
 ...
 it('Verify METRICS COUNT for Lambda in Test Branch', () => {
   ...
   jazzServices_po.getMetrices().click();
   commonUtils.waitForMetricsSpinner();
   commonUtils.refreshbutton(jazzServices_po.getMetricesCount(), tenk);
   expect(jazzServices_po.getMetricesCount().getText()).not.toEqual('-');
   browser.sleep(twok);
   commonUtils.fluentwaittry(jazzServices_po.getServiceHomePage(), fivek);
   jazzServices_po.getServiceHomePage().click();
